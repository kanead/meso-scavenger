...
#' load data
mydata <- read_csv("nf_1.csv", col_names = TRUE)
...
p1 <- ggplot(data = mydata1, aes(x = years , y = value, color = variable, fill = "species")) + geom_line(size=1.5)  +
...
p5 <- ggplot(data = mydata5, aes(x = years, y = value, color = variable)) + geom_line(size = 1.5) +
  ...
  labs(color="group") + #xlim(c(0,15000)) +
  scale_color_manual(labels = "Carrion N",values=c("#0072B2")) + ggtitle("C")
p5
